...
- Window `close` is intercepted (hide, not destroy); `window-all-closed` is a no-op; only the tray's Quit (or `before-quit`) actually quits. On real quit, `stopServer()` kills the **whole** server process group (`process.kill(-pid, 'SIGTERM')`), not just the immediate child — `tsx` spawns its own grandchild loader process, and killing only the wrapper leaked an orphaned server on quit until this was caught during verification.
