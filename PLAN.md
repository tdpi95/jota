...
The working directory (`/home/lap16152/Documents/Code/pivot`) currently has only an empty scaffold (`package.json`, `.gitignore`, empty dirs) plus a `design/` folder (a UI prototype canvas, see [design/README.md](design/README.md)) — nothing implemented yet.
...
- **App-level registry**: `~/.pivot/config.json` (outside any workspace, since the app needs to know the workspace list *before* opening one) — `{ workspaces: [{ id, path, name, lastOpenedAt }], activeWorkspaceId }`.
- **Per-workspace hidden directory**: `<workspace>/.pivot/` (mirrors Obsidian's `.obsidian/`), holding:
  ...
  - This directory is added to the workspace's own `.gitignore` (it's a cache, not content) but otherwise travels with the folder if copied — opening a copied/moved workspace elsewhere just works, or self-heals via reconciliation if `.pivot/` didn't come along.
- **Adding a workspace**: `POST /api/workspaces {path, name?}` — validates the path exists (or offers to create it), is readable/writable, scaffolds `.pivot/` and `projects/`/`journal/` subfolders if missing, `git init`s it if not already a repo, and registers it. Opening an *existing* folder that already has content just adopts it (like Obsidian's "Open folder as vault").
...
- **Preload script** (`electron/src/preload.ts`, `contextBridge` + `contextIsolation: true`, no `nodeIntegration` in the renderer) exposes a minimal `window.pivot` surface: `pickFolder()` → wraps `dialog.showOpenDialog`, returns the chosen path or `null`; `getLaunchAtLogin()` / `setLaunchAtLogin(bool)`; `getReminderSettings()` / `setReminderSettings({time, enabled})` for the active workspace. Everything else the client needs still goes over HTTP to the embedded server.
...
- Tasks: `POST /api/projects/:slug/tasks` (accepts `description`; server sets `createdAt`, `spentMinutes=0`), `PATCH /api/projects/:slug/tasks/:taskId` (status changes trigger the doing-timer transition logic; also updates `text`/`description`/`due`/`tags`), `DELETE .../tasks/:taskId`.
...
- Aggregates: `GET /api/tasks/open`, `GET /api/tasks/:taskId/journal-links`, `GET /api/tasks/search?q=`, `GET /api/calendar/:year/:month`, `GET /api/reports/time-spent?groupBy=&from=&to=&includeInProgress=`.
...
**Belt-and-suspenders**: a periodic full snapshot to `<workspace>/.pivot/backups/vault-<date>.zip` (daily, keep the last ~14) — cheap insurance against the git repo itself being deleted/corrupted; git history is still the primary recovery path.
...
- **Transport**: stdio, a standalone process (`server/src/mcp/index.ts`, built with `@modelcontextprotocol/sdk`), registered with an MCP host via `claude mcp add` or `.mcp.json` — no network port, no auth to manage. Takes an explicit target workspace (`PIVOT_WORKSPACE=<path>` env var, defaulting to the app registry's active workspace if omitted) so an agent's vault doesn't silently change if someone switches workspaces in the browser mid-conversation.
...
- **`AppShell`** — nav + **`WorkspaceSwitcher`** (list of known workspaces, "+ Open folder" wired to the native picker via `window.pivot.pickFolder()`, active-workspace indicator) + persistent **`CalendarSidebar`**: month grid backed by `GET /api/calendar/:year/:month`; marks days with a journal entry, and colored dots (by project color) for linked/due tasks; click navigates to `/journal/:year/:date`.
- `/` — **Dashboard**: open tasks across all projects, bucketed by due date, project color badges, "+ log to today" quick action per row.
- `/projects` — list with per-status task counts, tags, color accents, archived filter, new-project button.
- `/projects/:slug` — task list with description (expandable textarea), a `TimeSpentBadge` (live-ticking while doing), due date, tags; project edit form for description/tags/color; **History panel**.
...
- [server/src/lib/workspaces.ts](server/src/lib/workspaces.ts) — registry read/write, add/open/remove, `.pivot/` scaffolding; almost everything downstream operates against "the active workspace" this resolves.
...
2. **Workspace management** — `~/.pivot/config.json` registry, add/list/open/remove, `.pivot/` scaffold-on-open. Verify: register two scratch folders as workspaces, switch between them, confirm each gets its own `.pivot/` dir and the active one's routes serve its content; confirm removing a workspace doesn't touch its folder.
...
10. **MCP server** — `server/src/mcp/{index,tools}.ts`, explicit `PIVOT_WORKSPACE` targeting, registered via `claude mcp add`. Verify by driving it from a real MCP client: create/update tasks via tool calls, confirm files/index/git history all match what the REST API would have produced; deliberately make an "unwanted" change and confirm `POST /api/vault/revert/:commit` cleanly undoes it.
...
- Confirm portability end-to-end: manually edit a project `.md` file while the server is running, reload the UI, confirm changes appear; delete `<workspace>/.pivot/cache/index.sqlite3` entirely, reopen the workspace, and confirm full functionality with no data loss (simulates copying just the workspace folder — `.git` included — to a new machine).
