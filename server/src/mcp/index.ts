 ...
 * `JOTA_WORKSPACE` if set, else the app registry's currently-active
...
function resolveWorkspacePath(): string {
  const envPath = process.env.JOTA_WORKSPACE?.trim();
  if (envPath) return envPath;

  const active = getActiveWorkspace();
  if (!active) {
    throw new Error(
      'No workspace to target: set the JOTA_WORKSPACE environment variable to a workspace path, or open one in the jota app first.',
    );
  }
  return active.path;
}
...
async function main() {
  ...
  // "Workspaces": ".jota/ ... otherwise travels with the folder if
  ...
  const server = new McpServer({ name: 'jota', version: '0.1.0' });
  registerTools(server, workspacePath);

  const transport = new StdioServerTransport();
  await server.connect(transport);
  // stderr only — stdout is the JSON-RPC transport channel.
  console.error(`jota MCP server running on stdio, targeting workspace: ${workspacePath}`);
}

main().catch((err) => {
  console.error('jota MCP server failed to start:', err);
  process.exit(1);
});
