...
function scratchRepo(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pivot-vaultgit-'));
...
test('ensureGitHistory does not commit .pivot/ once it is gitignored', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pivot-vaultgit-'));
  fs.mkdirSync(path.join(dir, '.pivot', 'cache'), { recursive: true });
  fs.writeFileSync(path.join(dir, '.gitignore'), '.pivot/\n', 'utf8');
  fs.writeFileSync(path.join(dir, '.pivot', 'cache', 'index.sqlite3'), 'binary-ish', 'utf8');
